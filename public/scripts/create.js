...
socket.on('__search_results__', (msg) => {
    console.info(msg);
...
function option_action(cell, ev) {
    var _a;
    ev.stopPropagation();
    let id = (_a = cell.closest("table")) === null || _a === void 0 ? void 0 : _a.id;
    let row = cell.closest("tr");
    let content = row === null || row === void 0 ? void 0 : row.nextElementSibling;
    let extra_row = content === null || content === void 0 ? void 0 : content.nextElementSibling;
...
async function create_poll() {
    let rows = selectedOptionsElement.querySelectorAll("tr.option_row");
    let options = [];
...
socket.on('__update_configuration_complete__', (msg) => {
    search();
});
